...
Figure \ref{fig:iot_architecture} illustrates the typical three-layer architecture of IoT systems, showing the flow of data from perception devices through network gateways to cloud services.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/iot_architecture.png}
\caption{Three-layer IoT architecture showing the perception, network, and application layers with their respective security concerns}
...
Figure \ref{fig:iot_constraints} illustrates how different IoT device classes vary in their resource capabilities. Class 0 devices (8-bit microcontrollers) are the most constrained, while Class 2 devices (Linux-capable) have more resources but still face limitations compared to traditional computing platforms.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/iot_constraints.png}
\caption{Radar chart comparing resource capabilities across IoT device classes: Class 0 (8-bit MCU), Class 1 (32-bit MCU), and Class 2 (Linux-capable devices)}
